...
__global__ void kernel(float* dev_a, float* dev_b, int* dev_res, int n) {
	...
	for (int j = 0; j < n; ++j) {
		dev_res_private &= dev_b[col * n + j] == dev_a[col * n + (j + SHIFT) % n];
	}
	...
	__syncthreads();
	if (threadIdx.x == 0)
		atomicAnd(dev_res, dev_res_private);
}
...
int main(int argc, char** argv)
{
	...
	for (int i=0; i<device_count; i++)
	{
		...
		cout << i << ": " << dp.name << " with CUDA compute compatibility " <<
				dp.major << "." << dp.minor << "\n";
		cout << "maxGridSize : " << dp.maxGridSize[0] << "\t"
								   << dp.maxGridSize[1] << "\t"
								   << dp.maxGridSize[2] << "\t"<< endl;
		cout << "maxThreadsDim : " << dp.maxThreadsDim[0] << "\t"
								   << dp.maxThreadsDim[1] << "\t"
								   << dp.maxThreadsDim[2] << "\t"<< endl;

	}
	cout << endl;


	int n = 500, m = 50000, shift = 1;
	float* a = generateSimpleCyclicMatrix(n, m);
	float* b = generateCyclicShiftMatrix(a, n, m, shift);
	int res_CPU = true, * res_GPU = new int[1]{1};
	...
	kernel <<<gridSize, blockSize>>> (dev_a, dev_b, dev_res, n);
